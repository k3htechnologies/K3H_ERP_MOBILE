...
class CompanyMasterCubit extends Cubit<CompanyMasterState> {
    ...
    if (isClear ?? false) {
      ...
      emit(
        state.copyWith(
          ...
          filterByCinNumber: cinNumber ?? state.filterByCinNumber,
          filterByPanNumber: panNumber ?? state.filterByPanNumber,
          filterByTanNumber: tanNumber ?? state.filterByTanNumber,
          currentPage: 1,
        ),
      );
    }
  ...
  Future<void> getCompanyMaster(BuildContext context, int pageNumber) async {
    ...
    Map<String, dynamic> queryParams = {
      ...
      "CINNumber": state.filterByCinNumber,
      "PANNumber": state.filterByPanNumber,
      "TANNumber": state.filterByTanNumber,
    };
  ...
  int updateFilterCount(CompanyMasterState state) {
    ...
    return getActiveFilterCount([
      ...
      state.filterByCinNumber.trim().isNotEmpty,
      state.filterByPanNumber.trim().isNotEmpty,
      state.filterByTanNumber.trim().isNotEmpty,
      hasSort,
    ]);
  }
}
