...
import 'package:k3h_erp_app/features/masters/terms_and_condition_master/data/model/terms_and_condition.model.dart';
...
class TermsAndConditionCubit extends Cubit<TermsAndConditionState> {
  TermsAndConditionCubit() : super(TermsAndConditionState.initial());

  final TermsAndConditionMasterRepository _termsAndConditionsMasterRepository =
      serviceLocator<TermsAndConditionMasterRepository>();

  // <---- RESET STATE ---->
  void resetState() {
    emit(TermsAndConditionState.initial());
  }
  ...
  Future getMaterialRequisitionTermsAndConditionList(
    ...
    result.fold(
      ...
      (response) {
        List<TermsAndConditionModel> updatedList = List.from(
          state.materialRequisitionTermsAndConditionsList,
        );
        updatedList.addAll(response['data'] as List<TermsAndConditionModel>);
        emit(
          state.copyWith(
            isLoading: false,
  ...
  Future addMaterialRequisition({
    ...
    addResult.fold(
      ...
      (response) {
        goRouter.pop();
        var list = [
          response['data'][0] as TermsAndConditionModel,
          ...state.materialRequisitionTermsAndConditionsList,
        ];

        emit(
          state.copyWith(
            bookingTermsAndConditionsList: list,
            materialRequisitionTotalNumberOfRecordTermsAndConditions:
                state.materialRequisitionTotalNumberOfRecordTermsAndConditions ==
                        -1
                    ? 1
  ...
  Future updateMaterialRequisition({
    ...
    addResult.fold(
      ...
      (response) {
        final updatedList = List<TermsAndConditionModel>.from(
          state.materialRequisitionTermsAndConditionsList,
        );
        updatedList[index] = (response['data'][0] as TermsAndConditionModel);
        goRouter.pop();
  ...
  Future searchBooking(BuildContext context, String value) async {
    ...
    await getBookingTermsAndConditionList(context, 1, 20);
  }
      ...
      (response) {
        exportExcelOrPdfMobile(
          ...
          exportType.toLowerCase() == "pdf"
              ? "material_requisition_${DateTime.now()}.pdf"
              : "material_requisition_${DateTime.now()}.xlsx",
        );
      },
    );
  }
